...
const fileItem = tv({
  slots: {
    ...
    icon: 'rounded-full border-4 border-violet-100 bg-violet-200 p-2 text-violet-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-violet-400',
    deleteButton: '',
  },
      ...
      {state === 'error' ? (
        <div className="flex flex-1 flex-col items-start gap-1">
          <div className="flex flex-col">
            ...
            <span className="text-sm text-red-600 dark:text-error-600">
              {name}
            </span>
          </div>
          ...
          <button
            type="button"
            className="dark:hover-text-error-300 text-sm font-semibold text-error-700 hover:text-error-900 dark:text-error-400"
          >
            Try again
          </button>
        </div>
